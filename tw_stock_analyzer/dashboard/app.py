...
def main() -> None:
    ...
    with tab_chart:
        ...
        with _log.open("a", encoding="utf-8") as _f:
            _f.write(json.dumps({"sessionId": "938789", "hypothesisId": "D", "location": "app.py:tab_chart", "message": "bar gap check", "data": {"chart_timeframe": chart_timeframe, "chart_range": chart_range, "display_rows": len(display_df), "display_start": str(_idx.min()) if len(_idx) else None, "display_end": str(_idx.max()) if len(_idx) else None, "gaps_over_4d": _gaps, "gap_count_over_4d": len(_gaps), "x_axis": "category" if chart_spec.is_intraday or chart_timeframe in ("日線", "週線", "月線") else "datetime"}, "timestamp": int(time.time() * 1000)}) + "\n")
        # #endregion
